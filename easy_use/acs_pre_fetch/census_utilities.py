...
def get_acs(tract=None,state=None,county=None):
	out = {}
	codes,groups = get_lookup_codes_and_groups()
	url = build_census_query_url(tract=tract,state=state,county=county,codes=codes,groups=groups)
	r = requests.get(url)
...
def build_census_query_url(tract=None,state=None,county=None,codes=None,groups=None):
	...
	return f"{census_api_query_url}&for=tract:{tract}&in=state:{state}{countystr}&key={census_api_key}"

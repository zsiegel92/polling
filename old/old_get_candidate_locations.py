...
from place_types import interesting
...
def get_nearby_places(coords,n=10):
	...
	# fillerquery = "https://maps.googleapis.com/maps/api/place/textsearch/xml?query=restaurants+in+Sydney&key={api_key}"
	...
	for qtype in ['school','local_government_office','church','post_office']:
		nearbyquery = f"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location={coords[0]},{coords[1]}&radius={radius}&type={qtype}&key={api_key}"
...
def get_nearby_places_findplace_api(coords,n=10):
	...
	for qtype in ['school','local_government_office','church','post_office']:
		...
		# nearbyquery = f"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location={coords[0]},{coords[1]}&radius={radius}&type={qtype}&key={api_key}"
		findplacequery = f"https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input={qtype}&inputtype=textquery&fields=formatted_address,name,geometry,id,place_id&locationbias=circle:1000@{coords[0]},{coords[1]}&key={api_key}"
...
def get_place_info(row):
	...
	q = f"https://maps.googleapis.com/maps/api/place/findplacefromtext/json?inputtype=textquery&input={pollingLocationName}&locationbias=point:{coords[0]},{coords[1]}&fields=formatted_address,geometry,name,permanently_closed,place_id,plus_code,types&key={api_key}"
	r = requests.get(q)
	rdata = r.json()
	return rdata

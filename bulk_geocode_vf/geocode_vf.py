...
def get_rows_from_files(fnames):
	return [row for fname in fnames for row in get_rows(fname,safe_fast=True)] #get_rows(fname,safe=True)
...
def get_all_ohio_voters():
	print("GETTING OHIO VOTERS")
	voters = get_rows_from_files(ohio_vf_filenames)
	print("GOT OHIO VOTERS")
	return voters


def transform_counties_to_ohio_alphabetical(ohio_counties):
	return [ str(county_numbers[county['properties']['NAME']]) for county in ohio_counties ]
